...
p.add_argument("--mode", type=str, default="mlp", help='Options are "mlp" or "nerf"')
    ...
    def __init__(self):
        ...
        if opt.mode == "mlp":
            self.model = modules.SingleBVPNet(
                type=opt.model_type, final_layer_factor=1, in_features=3
            )
        elif opt.mode == "nerf":
            self.model = modules.SingleBVPNet(
                type="relu", mode="nerf", final_layer_factor=1, in_features=3
            )
