...
def gradient_fn(y, x, grad_outputs=None):
...
def sdf_loss(model_output, gt):
    ...
    gradient = gradient_fn(pred_sdf, coords)
...
def write_sdf_summary(
    ...
    with torch.no_grad():
        ...
        min_max_summary(
            prefix + "model_out_min_max", model_output["model_out"], writer, total_steps
        )
...
def densely_sample_activations(model, num_dim=1, num_steps=int(1e6)):
    ...
    if num_dim == 1:
        ...
        input = torch.stack(torch.meshgrid(*(input for _ in num_dim)), dim=-1).view(
            -1, num_dim
        )
...
def write_wave_summary(
    ...
    with torch.no_grad():
        ...
        for i in range(Nslice):
            pred = model({"coords": coords[:, i * split : (i + 1) * split, :]})[
                "model_out"
            ]
...
def write_helmholtz_summary(
    ...
    with torch.no_grad():
        if "coords_sub" in model_input:
            summary_model_input = {
                "coords": coords.repeat(
                    min(2, model_input["coords_sub"].shape[0]), 1, 1
                )
            }
        ...
        if "pretrain" in gt:
            gt["squared_slowness_grid"] = pred[..., -1, None].clone() + 1.0
            if torch.all(gt["pretrain"] == -1):
                gt["squared_slowness_grid"] = (
                    torch.clamp(pred[..., -1, None].clone(), min=-0.999) + 1.0
                )
        ...
        min_max_summary(prefix + "coords", model_input["coords"], writer, total_steps)
        min_max_summary(
            prefix + "pred_real", pred[..., 0::2, :, :], writer, total_steps
        )
        min_max_summary(
            prefix + "pred_abs",
    ...
    if isinstance(model, meta_modules.NeuralProcessImplicit2DHypernetBVP):
        hypernet_activation_summary(
            model, model_input, gt, model_output, writer, total_steps, prefix
        )


def write_image_summary_small(
    image_resolution,
    mask,
    model,
    model_input,
    ...
    with torch.no_grad():
        img_gradient = torch.autograd.grad(
            model_output["model_out"],
            [model_output["model_in"]],
            grad_outputs=torch.ones_like(model_output["model_out"]),
            create_graph=True,
            retain_graph=True,
        )[0]
...
def hypernet_activation_summary(
    model, model_input, gt, model_output, writer, total_steps, prefix="train_"
):
    with torch.no_grad():
        ...
        writer.add_histogram(
            prefix + "latent_code", embedding.cpu(), global_step=total_steps
        )
...
def write_image_summary(
    ...
    img_laplace = diff_operators.laplace(
        model_output["model_out"], model_output["model_in"]
    )
    ...
    pred_img = (
        dataio.rescale_img((pred_img + 1) / 2, mode="clamp")
        .permute(0, 2, 3, 1)
        .squeeze(0)
        .detach()
        .cpu()
    ...
    pred_grad = (
        dataio.grads2img(dataio.lin2img(img_gradient))
        .permute(1, 2, 0)
        .squeeze()
        .detach()
        .cpu()
    ...
    pred_lapl = cv2.cvtColor(
        cv2.applyColorMap(
            dataio.to_uint8(
                dataio.rescale_img(dataio.lin2img(img_laplace), perc=2)
                .permute(0, 2, 3, 1)
                .squeeze(0)
                .detach()
                .cpu()
    ...
    gt_img = (
        dataio.rescale_img((gt_img + 1) / 2, mode="clamp")
        .permute(0, 2, 3, 1)
        .squeeze(0)
        .detach()
        .cpu()
    ...
    gt_grad = (
        dataio.grads2img(dataio.lin2img(gt["gradients"]))
        .permute(1, 2, 0)
        .squeeze()
        .detach()
        .cpu()
    ...
    gt_lapl = cv2.cvtColor(
        cv2.applyColorMap(
            dataio.to_uint8(
                dataio.rescale_img(dataio.lin2img(gt["laplace"]), perc=2)
                .permute(0, 2, 3, 1)
                .squeeze(0)
                .detach()
                .cpu()
    ...
    write_psnr(
        dataio.lin2img(model_output["model_out"], image_resolution),
        dataio.lin2img(gt["img"], image_resolution),
        writer,
        total_steps,
        prefix + "img_",
    )
...
def write_laplace_summary(
    ...
    output_vs_gt = torch.cat(
        (dataio.rescale_img(gt_img), dataio.rescale_img(pred_img, perc=1e-2)), dim=-1
    )
    writer.add_image(
        prefix + "comp_gt_vs_pred",
        make_grid(output_vs_gt, scale_each=False, normalize=True),
        global_step=total_steps,
    )
    ...
    pred_laplace = diff_operators.laplace(
        model_output["model_out"], model_output["model_in"]
    )
    ...
    writer.add_image(
        prefix + "comp_gt_vs_pred_laplace",
    ...
    #                  global_step=total_steps)
    gt_laplace_img = dataio.to_uint8(
        dataio.to_numpy(dataio.rescale_img(gt_laplace, "scale", 1))
    )
    ...
    writer.add_image(
        prefix + "gt_lapl",
        torch.from_numpy(gt_laplace_img).permute(2, 0, 1),
        global_step=total_steps,
    )
    ...
    # Plot pred gradient
    pred_gradients = diff_operators.gradient(
        model_output["model_out"], model_output["model_in"]
    )
    ...
    #                  global_step=total_steps)
    pred_laplace_img = dataio.to_uint8(
        dataio.to_numpy(dataio.rescale_img(pred_laplace, "scale", 1))
    )
    ...
    writer.add_image(
        prefix + "pred_lapl",
        torch.from_numpy(pred_laplace_img).permute(2, 0, 1),
        global_step=total_steps,
    )
...
def write_gradients_summary(
    ...
    if "laplace" in gt:
        ...
        gt_laplace_img = dataio.to_uint8(
            dataio.to_numpy(dataio.rescale_img(gt_laplace_img, "scale", 1))
        )
        gt_laplace_img = cv2.applyColorMap(gt_laplace_img.squeeze(), cmapy.cmap("RdBu"))
        gt_laplace_img = cv2.cvtColor(gt_laplace_img, cv2.COLOR_BGR2RGB)
        writer.add_image(
...
def write_audio_summary(
    ...
    # write audio files:
    wavfile.write(
        os.path.join(logging_root_path, "gt.wav"),
        gt_rate,
        gt_func.detach().cpu().numpy(),
    )
    wavfile.write(
        os.path.join(logging_root_path, "pred.wav"),
        gt_rate,
        pred_func.detach().cpu().numpy(),
    )
