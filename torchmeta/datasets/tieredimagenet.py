...
class TieredImagenet(CombinationMetaDataset):
    ...
    def __init__(
        ...
        dataset = TieredImagenetClassDataset(
            root,
...
class TieredImagenetClassDataset(ClassDataset):
    ...
    def __init__(
        ...
        super(TieredImagenetClassDataset, self).__init__(
            meta_train=meta_train,
            meta_val=meta_val,
            meta_test=meta_test,
            meta_split=meta_split,
    ...
    def download(self):
        ...
        download_file_from_google_drive(
            self.gdrive_id, self.root, self.tar_filename, md5=self.tar_md5
        )
        ...
        for split in ["train", "val", "test"]:
            ...
            images_filename = os.path.join(
                tar_folder, "{0}_images_png.pkl".format(split)
            )
            ...
            with open(
                os.path.join(self.root, self.filename_labels.format(split)), "w"
            ) as f:
            ...
            with h5py.File(filename, "w") as f:
                ...
                for i, label in enumerate(tqdm(labels_str, desc=filename)):
                    ...
                    dataset.attrs["label_general"] = (
                        general_labels_str[general_idx]
                        if general_idx < len(general_labels_str)
                        else ""
                    )
